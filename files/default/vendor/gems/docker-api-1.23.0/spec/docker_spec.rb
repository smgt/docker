...
describe Docker do
  ...
  describe '#version' do
    ...
    let(:expected) do
      %w(ApiVersion Arch BuildTime GitCommit GoVersion KernelVersion Os Version)
    end
  ...
  describe '#info' do
    ...
    let(:keys) do
      %w(BridgeNfIp6tables BridgeNfIptables ClusterAdvertise ClusterStore
         Containers CpuCfsPeriod CpuCfsQuota Debug DockerRootDir Driver
         DriverStatus ExecutionDriver ExperimentalBuild HttpProxy HttpsProxy
         ID IPv4Forwarding Images IndexServerAddress I\nnitPath InitSha1
         KernelVersion Labels LoggingDriver MemTotal MemoryLimit NCPU
         NEventsListener NFd NGoroutines Name NoProxy OomKillDisable
